...
const SocialMediaSidebar = ({ data }) => {
  ...
  return (
    <div 
      className={`fixed left-0 top-1/2 transform -translate-y-1/2 flex flex-col space-y-2 p-2 bg-gray-800 text-white rounded-r-lg shadow-lg z-50 transition-all duration-300
    ...
    >
      {data.map((link, index) => (
        ...
        >
          <i className={`fab fa-${link.name.toLowerCase()} text-xl min-w-[24px] text-center`}></i>
